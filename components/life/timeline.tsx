'use client';
import {useState} from 'react';import {ArrowDownLeft,ArrowUpLeft,Check,Flame,Smile,Target} from 'lucide-react';import {dateOffset,money,todayKey,type Entry} from '@/lib/life-model';import {select,useLife} from './use-life';import {Empty} from './editor';
export type FeedType='expense'|'income'|'task'|'habit'|'checkin'|'goal';
...
const icons:Record<FeedType,typeof Check>={expense:ArrowDownLeft,income:ArrowUpLeft,task:Check,habit:Flame,checkin:Smile,goal:Target};
const groupsOf:Record<string,FeedType[]>={money:['expense','income'],task:['task'],habit:['habit'],checkin:['checkin','goal']};
...
export function TimelineView(){
  ...
  <div className="filter-row" aria-label="סינון ציר הזמן">{[['all','הכל'],['money','כספים'],['task','משימות'],['habit','הרגלים'],['checkin','רגעים אישיים']].map(([id,label])=><button key={id} aria-pressed={filter===id} className={filter===id?'selected':''} onClick={()=>setFilter(id)}>{label}</button>)}</div>
