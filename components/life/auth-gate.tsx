...
export function AuthGate({children}:{children:ReactNode}) {
  ...
  // This is also the server-rendered first paint, before anyone knows whether
  // Alon is signed in. Showing the sign-in card here flashed it on every open.
  if(status==='loading')return <main className="auth-shell" dir="rtl"><output className="app-status">מתחבר…</output></main>;
  ...
  {status==='error'?<button className="primary-action" onClick={()=>{setStatus('loading');void check();}}>נסה שוב</button>:<button className="google-action" onClick={()=>{window.location.assign('/api/auth/google');}}><LogIn size={18}/>התחברות עם Google</button>}
