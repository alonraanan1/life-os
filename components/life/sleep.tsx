...
export function SleepView({title}:{title?:string}){
  ...
  return <section className="sleep-view"><header className="module-header"><div><h2>{title||'איך אתה ישן'}</h2><p>{average?'ממוצע '+average+' בשבעת הלילות האחרונים':'שעות שינה, דירוג והיסטוריית לילות.'}</p></div><button className="quiet-action" onClick={()=>setEditing(null)}><Plus size={17}/>לילה חדש</button></header>
