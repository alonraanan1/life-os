...
export type HabitData={title:string;emoji:string;days:number[];startDate:string};
export type HabitEntryData={habitId:string;date:string;done:boolean};
export type TransactionData={title:string;category:string;date:string;amount:number;direction:'expense'|'income'};
...
case 'habit':{if(!Array.isArray(d.days)||d.days.length<1||d.days.length>7||d.days.some(x=>!Number.isInteger(x)||x<0||x>6))throw new Error('יש לבחור ימי ביצוע');result={title:text(d.title),emoji:text(d.emoji,12),days:[...new Set(d.days)],startDate:date(d.startDate)};break;}
case 'habitEntry':{const day=date(d.date);if(day>todayKey())throw new Error('לא ניתן לסמן הרגל בעתיד');result={habitId:text(d.habitId,100),date:day,done:bool(d.done)};break;}
case 'transaction':if(d.direction!=='expense'&&d.direction!=='income')throw new Error('סוג תנועה לא תקין');if(cents(d.amount)<=0)throw new Error('הסכום חייב להיות חיובי');result={title:text(d.title),category:text(d.category,80),date:date(d.date),amount:cents(d.amount),direction:d.direction};break;
