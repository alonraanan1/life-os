...
amount is in ILS (the app stores integer agorot). Use the same externalId when retrying so the expense is not duplicated. A 201 creates a record; 200 with created:false means already recorded. The endpoint is gated by the bearer key alone and no longer depends on an activated owner row. The key is provisioned separately as APPLE_SHORTCUTS_API_KEY and must never be committed. Owner's local copy is .env.shortcuts.txt. No Supabase dependency remains in this endpoint.
